...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  console.log('profileUrl', this.profileUrl, 'accessToken', accessToken);
  var me = {
    url: 'https://api.linkedin.com/v2/me?fields=id,first-name,last-name,email-address,formatted-name,picture-url',
    headers: {
      'Authorization': 'Bearer ' + accessToken
    }
  };
  console.log('me.url', me.url);
  request.get(me, function(err, response, body) {
    console.log('err', err, arguments.length);
    console.log(body)
  });
  var pfurl = this.profileUrl;
  this._oauth2.get(this.profileUrl, accessToken, function (err, body, res) {
    console.log('passport-linkedin-oauth2', { profileUrl: pfurl, accessToken });
